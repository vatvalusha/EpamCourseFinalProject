        ...
        <c:otherwise>
            ...
            <form action="routes" accept-charset="UTF-8" method="post">
                <table style="margin: auto">
                    ...
                    <c:forEach items="${transports}" var="unit">
                        <tr>
                            <td><c:out value="${unit.getID_TRANSPORT()}"></c:out></td>
                            <td></td>
                            <td><c:out
                                    value="${language eq 'ru' ? unit.getMODEL_NAME_RU() : unit.getMODEL_NAME_EN()}"></c:out></td>
                            <td></td>
                            <td><c:out
                                    value="${language eq 'ru' ? unit.getTypeTransport().getNameType() : unit.getTypeTransport().getNameType()}"></c:out></td>
                            <td></td>
                        </tr>
                    </c:forEach>
                </table>
